...
brazilian_population_df = load_data(
...
st.markdown(
    """<a href="https://www.kaggle.com/datasets/olistbr/brazilian-ecommerce/">👉 Original Kaggle Dataset 🔗</a>""",
    unsafe_allow_html=True,
)
st.markdown(
    """<a href="https://www.pola.rs/">👉 Polars 🔗</a>""",
    unsafe_allow_html=True,
)
st.text("")
st.markdown("For this project, we decided to push the limits of data processing.")
st.markdown(
    "Using the Polars library, in combination with Pandas, we were able to clean, transform, join, and reshape 8 files containing anywhere from **:blue[20 to 100K records]**  in less than 5-6 min. "
)
st.markdown(
...
with tab1:
    ...
    col3.metric(
        label="Avg Processing Time for The Data & Modeling Pipeline Combined",
        value="<5 Min",
    )
    ...
    with st.container():
        ...
        with col6:
            st.metric(
                ...
                value=round(report_profile_df["Time in Mins"].sum(), 2),
            )
...
with tab2:
    with st.container():
        ...
        orders_df["order_purchase_timestamp"] = pd.to_datetime(
            orders_df["order_purchase_timestamp"], format="%Y-%m-%d %H:%M:%S"
        )
        vol_df = orders_df.resample(rule="M", on="order_purchase_timestamp")[
            "order_id"
        ].count()
        vol_df = (
            vol_df.to_frame()
            ...
            .rename(
                columns={
                    "order_id": "Vol. Orders (Count)",
                    "order_purchase_timestamp": "Order Purchase (Month - Year)",
                }
            )
        )
    ...
    with st.container():
        ...
        vol_region_df = (
            pl.DataFrame(orders_df)
            .join(pl.DataFrame(customers_df), on="customer_id")
            .select(
                [
                    ...
                    .count()
                    .over("customer_state")
                    .alias("Vol Orders Per State"),
                ]
            )
            .unique()
            .sort("Vol Orders Per State", descending=True)
            .to_pandas()
        )
        ...
        vol_region_df = (
            pl.DataFrame(vol_region_df)
            .join(
                pl.DataFrame(states_abbrev_df),
                left_on="Customer State",
                right_on="Code",
            )
            .to_pandas()
        )
    ...
    with st.container():
        ...
        predictions_df["order_purchase_timestamp"] = pd.to_datetime(
            predictions_df["order_purchase_timestamp"], format="%Y-%m-%d %H:%M:%S"
        )

        predicted_delivery_duration = predictions_df.resample(
            rule="M", on="order_purchase_timestamp"
        )["predictions"].mean()

        actual_delivery_duration = predictions_df.resample(
            rule="M", on="order_purchase_timestamp"
        )["label_actual_delivery_duration"].mean()

        original_estimate_delivery_duration = predictions_df.resample(
            rule="M", on="order_purchase_timestamp"
        )["label_estimated_delivery_duration"].mean()

        all_series = pd.concat(
            [
                predicted_delivery_duration,
                actual_delivery_duration,
                original_estimate_delivery_duration,
            ],
            axis=1,
        ).reset_index()
        ...
        fig3.add_traces(
            go.Scatter(
                x=all_series["order_purchase_timestamp"],
                y=all_series["predictions"],
                mode="lines",
                name="RF Predicted Duration",
        ...
        fig3.add_traces(
            go.Scatter(
                x=all_series["order_purchase_timestamp"],
                y=all_series["label_actual_delivery_duration"],
                mode="lines",
        ...
        fig3.add_traces(
            go.Scatter(
                x=all_series["order_purchase_timestamp"],
                y=all_series["label_estimated_delivery_duration"],
                mode="lines",
                name="OG Estimated Duration",
...
with tab4:
    ...
    st.markdown(
        "Here we'll predict the delivery times based on a couple factors for a single order."
    )
    ...
    with st.form("try model"):
        feat_total_payment_value = st.number_input(
            "Total Order Value",
            min_value=1,
            max_value=20000,
            key="feat_total_payment_value_DROPDOWN",
        )
        feat_num_items_per_order = st.number_input(
            "No. of Items",
            min_value=1,
            max_value=30,
            key="feat_num_items_per_order_DROPDOWN",
        )
        feat_num_cat_per_order = st.number_input(
            "No. of Product Categories",
            min_value=1,
            max_value=5,
            key="feat_num_cat_per_order_DROPDOWN",
        )
        feat_num_sellers_per_order = st.number_input(
            "No. of Sellers",
            min_value=1,
            max_value=5,
            key="feat_num_sellers_per_order_DROPDOWN",
        )
        feat_num_seller_cities_per_order = st.number_input(
            "No. Seller Locations",
            min_value=1,
            max_value=5,
            key="feat_num_seller_cities_per_order_DROPDOWN",
        )
        feat_customer_holiday_impact = st.number_input(
            "No. Current Holidays In Customer Region",
            min_value=0,
            max_value=3,
            key="feat_customer_holiday_impact_DROPDOWN",
        )
        feat_seller_holiday_impact = st.number_input(
            "No. Current Holidays In All Seller Regions",
            min_value=0,
            max_value=15,
            key="feat_seller_holiday_impact_DROPDOWN",
        )
...
with tab5:
    ...
    st.markdown(
        "Here we use pandas-profiler to understand the relationships between the different features of our model and where we can improve."
    )
